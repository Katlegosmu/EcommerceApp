$(document).ready(function(){
	...
	*/
	...
	async function loadModel() {
		...
		if (!loggedInUserId) {
			console.warn("User not logged in. Cannot fetch recommendations.");
			return;
		}
	
		const recommendedProducts = await getRecommendations(model, loggedInUserId);
	...
	async function getRecommendations(model, userId) {
		...
		const productTensor = tf.tensor2d([...Array(10).keys()]); // Assuming 10 products for simplicity
	
		const predictions = model.predict([userTensor, productTensor]);
		const scores = await predictions.data();
	
		return scores.map((score, i) => ({ productId: i + 1, score }))
		...
		recommendations.forEach((item) => {
			container.innerHTML += `
				<div class="product-card">
					<p>Recommended Product ID: ${item.productId} (Score: ${item.score.toFixed(2)})</p>
				</div>
			`;
		});
	}
	...
	//AI feedback
	function sendFeedback(productId, feedback) {
		fetch('store_feedback.php', {
			method: 'POST',
			body: JSON.stringify({ product_id: productId, feedback: feedback }),
