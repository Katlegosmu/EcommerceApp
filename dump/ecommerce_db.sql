...
--
-- Host: 127.0.0.1
-- Generation Time: Apr 02, 2025 at 01:31 PM
...
(8, 'Katlego ', 'katlego@gmail.com', '$2y$10$YKSDtra7v2wH6ORYfry8Ue9t49pk1AvQvdJGuq4lDvFLEcx.kP6Mq', '1');
...
(1, 12, 2, 'Samsung Galaxy Z Fold 2', 2500, 5, 'Last yearâ€™s Galaxy Fold was a sort of experiment in the field of foldable phones. The idea was an innovative one but the phone faced a lot of durability issues. Its launch was postponed multiple times because of Samsungâ€™s inability to solve all the problems. Samsung will likely avoid those situations with its successor.', '1616500092_sm-zfold.jpg', 'samsung, mobile, galaxy fold'),
(2, 12, 3, 'Iphone 12 Pro Max', 5000, 7, '5G goes Pro. A14 Bionic rockets past every other smartphone chip. The Pro camera system takes low-light photography to the next level â€” with an even bigger jump on iPhone 12 Pro Max. And Ceramic Shield delivers four times better drop performance.', '1616499931_iph12pm.jpg', 'apple, iphone'),
(4, 12, 2, 'Samsung Galaxy S21 Ultra', 7000, 10, 'This is a demo', '1616492395_Samsung-Galaxy-S21-Ultra-1608287647-0-0.jpg', 'samsung, s21, s21 ultra'),
(5, 12, 6, 'OnePlus 8T', 8600, 13, 'On spec-sheet, the OnePlus 8T boasts plenty of improvements from its predecessor i.e. the OnePlus 8. For instance, its 6.55-inch 1080p OLED display now comes with a faster 120Hz refresh rate. In comparison, the OnePlus 8 had a 90Hz refresh rate. This upgrade seems huge. However, users will agree that you canâ€™t really find much of a difference between 90Hz to 120Hz on a smartphone screen.', '1616500410_OnePlus-8T-5G-Lunar-Silver-8GB-RAM-128GB-Storage-image-4.jpg', 'one plus, oneplus8'),
...
(11, 6, 9, 'Dr. Martens Mens Patch', 1600, 3, 'Color: Grey/Charcoal/Dark Grey', '1616503181_Dr. Martens.jpg', 'dr martens, shoes'),
...
(7, 'Katlego', 'Phoko', 'katlego@gmail.com', 'a6668e91a590eca0fa7638219c98c4d0', '0835741092', 'Faerie Glen , 1, Sprokie Avenue, Garsfontein, Tshwane', 'Faerie Glen');
...
CREATE TABLE `user_interactions` (
  ...
  `user_id` int(11) NOT NULL,
  `product_id` int(11) NOT NULL,
  `interaction_type` enum('view','click','purchase') NOT NULL,
...
--
ALTER TABLE `product_recommendations`
  ADD CONSTRAINT `product_recommendations_ibfk_1` FOREIGN KEY (`user_id`) REFERENCES `user_info` (`user_id`),
  ADD CONSTRAINT `product_recommendations_ibfk_2` FOREIGN KEY (`recommended_product_id`) REFERENCES `products` (`product_id`);
...
ALTER TABLE `user_feedback`
  ...
  ADD CONSTRAINT `user_feedback_ibfk_2` FOREIGN KEY (`recommended_product_id`) REFERENCES `products` (`product_id`);

--
-- Constraints for table `user_interactions`
--
ALTER TABLE `user_interactions`
  ADD CONSTRAINT `user_interactions_ibfk_1` FOREIGN KEY (`user_id`) REFERENCES `user_info` (`user_id`),
  ADD CONSTRAINT `user_interactions_ibfk_2` FOREIGN KEY (`product_id`) REFERENCES `products` (`product_id`);
COMMIT;
